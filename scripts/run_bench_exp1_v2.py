#!/usr/bin/env python3
"""Exp1: Cross-Benchmark Comparison on core benchmark (4,779 TRA/TRB pairs, 384 epitopes).

Runs all single methods + 3 consensus modes on the high-confidence benchmark.
"""
...
def load_benchmark():
    """Load core benchmark: 4,779 TRA/TRB pairs from R-filtered dataset."""
    ...
    log.info(f"Loaded benchmark: {len(df)} rows, {df['epitope'].nunique()} epitopes")
    return df
...
def run():
    ...
    print("\n" + "=" * 80)
    print("EXP1: CROSS-BENCHMARK RESULTS (4,779 pairs, 384 epitopes)")
    print("=" * 80)
